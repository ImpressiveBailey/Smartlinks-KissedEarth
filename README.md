...
## Installing on an impressive client

Login to shopify partner using analytics@impressive.com.au
Select the second Impressive Digital
Create a new app
Go to this repo
update `/shopify.app.toml`
Update the links, client id, dev_store_url
Do yarn deploy

Go to Hosting platform
Create a new container
Use ImpressiveRelatedCollection as template
Change the following:

- Name (!!!IMPORTANT!!! if you don't change this, it will override the existing container)
- API_KEY -> client id from shopify
- API_SECRET -> Client secret from shopify
- Port
- SHOPIFY_APP_URL
- App -> Delete this and recreate it so it's a new port number that's unused (Put new port number inside Container Port and Host Port then TCP)

Go to Cloudflare -> Zero Trust -> Networks -> Tunnels
Add new entry for the container

Once all setup, install it on the client site, generate the related collections. and apply
If it throws an error on the apply step, just refresh the page and try again.

Find the collections page template and paste this in:

!!!!You can find the metafield ID under settings.!!!!!

````
<!-- RELATED COLLECTION SNIPPET START -->
  {% assign related_collections = collection.metafields.app--253601579009.imp_related_collections.value %}
  {% assign excluded_collections = collection.metafields.app--253601579009.imp_excluded_collections.value %}
  {% assign additional_collections = collection.metafields.app--253601579009.imp_additional_collections.value %}
  {% assign related_collections_handles = related_collections | map: 'handle' %}
  {% assign excluded_collections_handles = excluded_collections | map: 'handle' %}

  {% if related_collections %}
    <div class="wrapper page-width">
      <h3>Related collections</h3>
      <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
        {% comment %} Loop through related collections and check if the handle is not in excluded_collections_handles {% endcomment %}
        {% for related_collection in related_collections %}
          {% unless excluded_collections_handles contains related_collection.handle %}
            <div><a class="btn btn--primary" href="{{ related_collection.handle }}">{{ related_collection.title }}</a></div>
          {% endunless %}
        {% endfor %}

        {% comment %} Loop through additional collections and check if the handle is not in related_collections_handles or excluded_collections_handles {% endcomment %}
        {% for additional_collection in additional_collections %}
          {% unless related_collections_handles contains additional_collection.handle or excluded_collections_handles contains additional_collection.handle %}
            <div><a class="btn btn--primary" href="{{ additional_collection.handle }}">{{ additional_collection.title }}</a></div>
          {% endunless %}
        {% endfor %}
      </div>
    </div>
